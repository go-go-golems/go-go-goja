...
- Generate native module declarations with `make gen-dts`.
...
| `make check-dts` fails | Generated declaration file drifted | Run `make gen-dts`, review diff, and recommit |
...
- `gen-dts`: regenerate native module declaration file.
- `check-dts`: fail if generated declarations differ from committed output.
...
- Run `make gen-dts` and `make check-dts` to ensure native module declarations are up to date.
