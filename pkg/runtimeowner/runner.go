...
func (r *runner) invoke(ctx context.Context, op string, fn CallFunc) (ret any, err error) {
...
func (r *runner) normalizeContext(ctx context.Context) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	if r.opts.MaxWait > 0 {
		if _, ok := ctx.Deadline(); !ok {
			c, _ := context.WithTimeout(ctx, time.Duration(r.opts.MaxWait)*time.Millisecond)
			ctx = c
		}
	}
	return ctx
