...
func TestServiceFactoryOIDCBuildsNativeHandlers(t *testing.T) {
	...
	for _, want := range []string{"GET /auth/login", "GET /auth/callback", "POST /auth/logout", "GET /auth/logout", "GET /auth/session", "POST /orgs/o1/invites", "POST /org-invites/accept"} {
		if !got[want] {
