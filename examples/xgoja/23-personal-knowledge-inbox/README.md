...
The Keycloak smoke remains available as a compatibility check. The tinyidp smokes default to root issuer URLs such as `http://127.0.0.1:19087`, but the Makefiles also accept a `TINYIDP_ISSUER` override for path-based issuers when using a tinyidp build that supports Keycloak-shaped routes. They also pass `tinyidp-users.yaml` so Alice and Bob have stable seeded subjects and app-specific claims. Step 08 uses tinyidp only for browser login; device authorization remains implemented by the generated xgoja host.

By default the step Makefiles expect the tinyidp checkout in the workspace sibling directory `../2026-06-22--mock-oidc-idp`. Override that with `TINYIDP_ROOT=/path/to/tinyidp make tinyidp-smoke` when running from a different checkout layout. To test a Keycloak-shaped issuer, pass for example `TINYIDP_ISSUER=http://127.0.0.1:19087/realms/personal-inbox` with matching free ports.
