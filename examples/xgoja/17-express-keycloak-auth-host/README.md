...
For command-line testing, use a browser or `curl` with a cookie jar to complete the Keycloak form login. After login, `GET /auth/session` returns the app session's CSRF token:
